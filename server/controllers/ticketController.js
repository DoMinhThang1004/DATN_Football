...
const getMyTickets = async (req, res) => {
  try {
    ...
    const query = `SELECT DISTINCT ON (t.id)
        t.id, t.status, t.qr_code_string, t.seat_number,
        m.home_team, m.away_team, m.start_time, m.home_logo, m.banner_url,
        s.name as stadium_name, tt.name as ticket_type,
        mtc.price, sz.zone_name  
      FROM tickets t
...
const getOccupiedSeats = async (req, res) => {
    try {
        ...
        const query = `SELECT seat_number FROM tickets WHERE match_config_id = $1 AND status IN ('VALID', 'USED')`;
