...
export default function MatchTicketConfig() {
  ...
  const [formData, setFormData] = useState({
    typeId: "", zoneId: "", price: 0, quantity: 0
  });
  ...
  const fetchAllData = async () => {
    try {
        ...
        if (!resMatches.ok || !resTypes.ok || !resZones.ok || !resConfigs.ok) {
        ...
        // Init Form mặc định nếu chưa edit
        if (!editingId) {
             if (types.length > 0) {
                setFormData(prev => ({ ...prev, typeId: types[0].id, price: Number(types[0].base_price) }));
            }
            if (zones.length > 0) {
  ...
  const handleTypeChange = (e) => {
    ...
    // Chỉ tự động điền giá nếu không phải đang sửa (hoặc muốn override)
    // Ở đây mình để tự động điền giá gợi ý
    setFormData(prev => ({
      ...prev,
      typeId: typeId,
      price: selectedType ? Number(selectedType.base_price) : 0
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.quantity <= 0) { showNotification("Số lượng phải lớn hơn 0!", "error"); return; }
    ...
    try {
        ...
        if (editingId) {
            // API PUT
            response = await fetch(`${API_BASE}/match-t-configs/${editingId}`, {
                method: 'PUT',
            ...
            // API POST
            response = await fetch(`${API_BASE}/match-t-configs`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
        ...
        if (response.ok) {
            ...
            fetchAllData(); 
            if (editingId) handleCancelEdit(); // Nếu đang sửa thì thoát chế độ sửa
            else setFormData(prev => ({ ...prev, quantity: 0 })); // Nếu thêm mới thì chỉ reset số lượng
        } else {
  ...
  const handleDeleteExecute = async () => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            fetchAllData();
            if (editingId === configToDelete.id) handleCancelEdit(); // Nếu đang sửa cái bị xóa thì reset
        } else {
            const result = await response.json();
        ...
        <div className="flex items-center gap-4 mb-6">
            ...
            <div>
                <h1 className="text-2xl font-bold text-gray-800">
                    Cấu hình vé: {matchInfo?.home_team} vs {matchInfo?.away_team}
                </h1>
                <p className="text-sm text-gray-500 flex items-center gap-2 mt-1">
                    <MapPin size={14}/> {matchInfo?.stadium_name || "Sân chưa xác định"} 
                </p>
            </div>
        </div>
                ...
                <div className={`bg-white p-6 rounded-xl shadow-sm border sticky top-24 transition-all ${editingId ? 'border-blue-500 ring-1 ring-blue-500' : 'border-blue-100'}`}>
                    ...
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                            ...
                            {editingId ? "Chỉnh sửa cấu hình" : "Phát hành vé mới"}
                        </h3>
                        {editingId && (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <select className="w-full border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500 bg-white" value={formData.typeId} onChange={handleTypeChange}>
                                {availableTypes.map(t => (<option key={t.id} value={t.id}>{t.name} (Gốc: {Number(t.base_price).toLocaleString()}đ)</option>))}
                            </select>
                        </div>
                        <div>
                            ...
                            <select className="w-full border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500 bg-white" value={formData.zoneId} onChange={e => setFormData({...formData, zoneId: e.target.value})}>
                                {stadiumZones.length === 0 ? (<option value="" disabled>⚠️ Sân này chưa có khu vực nào!</option>) : (<><option value="" disabled>-- Chọn khu vực --</option>{stadiumZones.map(z => (<option key={z.id} value={z.id}>{z.zone_name} (Sức chứa: {z.capacity})</option>))}</>)}
                            </select>
                        </div>
                        ...
                        <div><label className="block text-sm font-medium text-gray-700 mb-1">Số lượng phát hành</label><input type="number" className="w-full border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500" value={formData.quantity} onChange={e => setFormData({...formData, quantity: parseInt(e.target.value) || 0})} /></div>
                        
                        <button type="submit" className={`w-full text-white py-2 rounded-lg font-medium shadow-sm flex items-center justify-center gap-2 transition-colors ${editingId ? 'bg-blue-600 hover:bg-blue-700' : 'bg-green-600 hover:bg-green-700'}`}>
                            <Save size={18}/> {editingId ? "Lưu thay đổi" : "Lưu cấu hình"}
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div className="lg:col-span-2">
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                        ...
                        <div className="text-sm text-gray-500">Tổng: <span className="font-bold text-blue-600">{ticketConfigs.reduce((acc, curr) => acc + curr.quantity, 0).toLocaleString()}</span> vé</div>
                    </div>
                    {ticketConfigs.length > 0 ? (
                        <table className="w-full text-left">
                            <thead className="bg-white text-gray-500 text-sm border-b">
                                <tr><th className="p-4">Loại vé</th><th className="p-4">Khu vực</th><th className="p-4">Giá bán</th><th className="p-4 text-center">SL</th><th className="p-4 text-right">Đã bán</th><th className="p-4 text-right">Hành động</th></tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50 text-sm">
                                {ticketConfigs.map(config => (
                                    <tr key={config.id} className={`hover:bg-gray-50 transition-colors ${editingId === config.id ? 'bg-blue-50' : ''}`}>
                                        <td className="p-4 font-medium text-blue-600 flex items-center gap-2"><Tag size={16} className="text-gray-400"/> {config.typeName} <span className="w-3 h-3 rounded-full border shadow-sm" style={{backgroundColor: config.colorCode}}></span></td>
                                        <td className="p-4 text-gray-800">{config.zoneName}</td>
                                        <td className="p-4 font-bold text-green-600">{Number(config.price).toLocaleString()} đ</td>
                                        ...
                                        <td className="p-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <button onClick={() => handleEditClick(config)} className="text-blue-600 hover:bg-blue-50 p-2 rounded-lg transition" title="Sửa">
                                                    <Edit size={18}/>
                                                </button>
                                                <button onClick={() => confirmDelete(config)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg transition" title="Xóa" disabled={config.sold > 0}>
                                                    <Trash2 size={18}/>
                                                </button>
                                            </div>
                                        </td>
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-center">
                ...
                <p className="text-gray-500 text-sm mb-6">Bạn có chắc muốn xóa lô vé <strong>{configToDelete?.typeName} - {configToDelete?.zoneName}</strong>?</p>
